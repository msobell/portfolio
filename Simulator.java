...
public class Simulator implements Server.ClientHandler {
  ...
  HashMap<Player, Server.Client> players2clients = new HashMap<Player, Server.Client>();
  HashMap<Server.Client, Player> clients2players = 
    new HashMap<Server.Client, Player>();
    ...
    if (viz) {
      ...
      for (int i = 0; i < nRuns; i++) {
        ...
        if ((chance < do_it) && (changeCount < MAX_CHANGE) && (i > 0)) {
          System.out.println("Changing attributes for the " + (changeCount + 1)
              + " time!");
          assignHiddenAttrs();
          changeCount++;
        }
